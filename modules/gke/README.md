...
| authenticator\_groups\_config | Configuration for the Google Groups for GKE feature. | <pre>object({<br>  })</pre> | `null` | no |
...
| cluster\_name | name of GKE cluster to provision | `string` | n/a | yes |
...
| node\_pools | List of node pools configuration | <pre>list(object({<br>    node_count = number<br>  }))</pre> | `[]` | no |
...
| private\_cluster\_config | Configuration for private clusters, clusters with private nodes. | <pre>object({<br>  })</pre> | `null` | no |
